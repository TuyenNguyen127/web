...
const staticByMonthInBackAgent = async(req,res) => {
  ...
  try {
    ...
    let arr = new Array;
    let k = 0;
    for (let i = 0; i < back_agent.length; i++) {
      if (!(back_agent[i].time.getUTCMonth() - back_agent[i-1].time.getUTCMonth())) k++; 
      else {
        arr[k].push(back_agent[i]);
      }
    }
    return res.json({
      success: 1,
...
const staticByYearInBackAgent = async(req,res) => {
  ...
  try {
    ...
    let k = 0;
    for (let i = 0; i < back_agent.length; i++) {
      if (!(back_agent[i].time.getUTCFullYear() - back_agent[i-1].time.getUTCFullYear())) k++; 
      else {
        arr[k].push(back_agent[i]);
      }
    }
...
const staticByMonthInErService = async(req,res) => {
  ...
  try {
    const er_service = await erService.find({id_user: req.querry.id});
    er_service.sort(sortFunction);
    let arr = new Array;
    let k = 0;
    for (let i = 0; i < er_service.length; i++) {
      if (!(er_service[i].time.getUTCMonth() - er_service[i-1].time.getUTCMonth())) k++; 
      else {
        arr[k].push(er_service[i]);
      }
    }
    return res.json({
      success: 1,
...
const staticByYearInErService = async(req,res) => {
  ...
  try {
    ...
    let k = 0;
    for (let i = 0; i < er_service.length; i++) {
      if (!(er_service[i].time.getUTCFullYear() - er_service[i-1].time.getUTCFullYear())) k++; 
      else {
        arr[k].push(er_service[i]);
      }
    }
